...
static int samsung_panel_on_post(struct samsung_display_driver_data *vdd)
{
	...
	pcmds = ss_get_cmds(vdd, TX_VRR_GM2_GAMMA_COMP3); // 65 + 3 bytes
	if (!SS_IS_CMDS_NULL(pcmds)) {
		...
		LCD_INFO(vdd, "write 60HS gamma offset \n");
	...
	spsram_read_bytes(vdd, 0x08A2, GAMMA_R_SIZE, readbuf);
	ss_send_cmd(vdd, TX_POC_DISABLE);

	for (i = 0; i < GAMMA_R_SIZE; i++)
		snprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", readbuf[i]);
	LCD_INFO(vdd, "[08A2] %s\n", pBuffer);

	for (i = 0; i < GAMMA_R_SIZE; i++) {
		if (readbuf[i] != HS60_R_TYPE_COMP[73][i]) {
			LCD_ERR(vdd, "SRAM_WRITE_FAIL!! [%d] %02x %02x \n", i, readbuf[i], HS60_R_TYPE_COMP[73][i]);
			panic("sram fail");
		}
	}
#endif
